...
int main(int argc,  char *argv[ ]) {
   ...
   long array_ids[nprocs];
   array_ids[0] = (long)getpid();
   long next_id = (long)getpid();
   int k;
   for(k = 1; k < nprocs; k++){
      write(STDOUT_FILENO, &next_id, sizeof(long));
      read(STDIN_FILENO, &next_id, sizeof(long));
      array_ids[k] = next_id;
   }
